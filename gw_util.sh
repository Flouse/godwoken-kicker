...
cargo_build_local_or_docker(){
    if [[ "$BUILD_GODWOKEN_ON_LOCAL_OVER_DOCKER" = true ]]; then
        ...
        if [[ $(docker -v) != *20.10.* ]] && [[ "$OSTYPE" == "linux-gnu"* ]]; then
            ...
            docker run --rm -i --network="host" -v `pwd`/packages/godwoken:/app -v `pwd`/docker/layer2/cargo:/.cargo/ -v `pwd`/cache/build/cargo-registry:/root/.cargo/registry -w=/app retricsu/godwoken-manual-build cargo build ;
            return 0;
        fi

        if [[ "$OSTYPE" == "linux-gnu"* ]]; then
            echo "linux-gnu"
            docker run --rm -i --add-host host.docker.internal:host-gateway -v `pwd`/packages/godwoken:/app -v `pwd`/docker/layer2/cargo:/.cargo/ -v `pwd`/cache/build/cargo-registry:/root/.cargo/registry -w=/app retricsu/godwoken-manual-build cargo build ;
        elif [[ "$OSTYPE" == "darwin"* ]]; then
            # Mac OSX
            echo "mac osx"
            docker run --rm -i -v `pwd`/packages/godwoken:/app -v `pwd`/docker/layer2/cargo:/.cargo/ -v `pwd`/cache/build/cargo-registry:/root/.cargo/registry -w=/app retricsu/godwoken-manual-build cargo build ;
